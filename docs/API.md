...
`409` if the tournament is full or the caller is already in it. `400` if it has
already started, is team-based, is application-gated, the caller is the host, or
...
`409` if the caller (or their team) has already applied. `400` if the tournament
is open to all, has started, or the team is not the right size.
...
`409` if every slot is already taken or promised to another accepted applicant.
